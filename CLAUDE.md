...
⚠️ O **Norton AntiVirus desta maquina** intercepta TLS — `REQUESTS_CA_BUNDLE=~/.azure/cacert-corp.pem`
para o az, `NODE_EXTRA_CA_CERTS` para o Node, senao tudo falha com erro de certificado. Ver a secao
"Norton AntiVirus intercepta TLS" abaixo.
...
### Norton AntiVirus intercepta TLS nesta maquina e quebra TODA ferramenta que baixa algo

**Nao e a rede corporativa** — essa foi uma atribuicao errada minha, corrigida em 2026-09-09. O
interceptador e **local**: o Norton AntiVirus (servico `NortonSvc`, `C:\Program Files\Norton\Suite`)
faz "SSL/TLS scanning" e reassina todo HTTPS com o CA `CN=Norton Web/Mail Shield Root`. Para conferir:
...
**A correcao de raiz e desligar o HTTPS scanning do Norton** (ou excluir os hosts de build), nao
contornar em cada ferramenta. Enquanto isso nao for feito valem os workarounds abaixo. O CA do Norton
**esta** no store do Windows — por isso o navegador funciona —, mas **cada runtime tem truststore
proprio** e nenhum consulta o do Windows por padrao:

| Runtime | Sintoma | Correcao |
|---|---|---|
| Python (az CLI) | `CERTIFICATE_VERIFY_FAILED` | `REQUESTS_CA_BUNDLE=~/.azure/cacert-corp.pem` |
| Java (Gradle, downloads grandes) | `(bad_record_mac) Tag mismatch!` / `Illegal packet size` | `-Djdk.tls.client.protocols=TLSv1.2` + `--max-workers=2` — o Norton corrompe o stream em TLS 1.3 |
