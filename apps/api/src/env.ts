...
const schema = z.object({
  ...
  JWT_ISSUER: z.string().default("Wityu"),
  JWT_AUDIENCE: z.string().default("Wityu.App"),
  ...
  // hand a one-time code back to the app via the wityu:// deep link.
  ...
  // the wityu:// deep link; the web build needs a real http origin (a browser
  // cannot open wityu://, which is why web sign-in used to hang forever).
  OAUTH_REDIRECT_ALLOWLIST: z
    .string()
    .default("wityu://auth/callback,http://localhost:8081/auth-callback"),
